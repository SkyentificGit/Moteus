...
class MoteusReg():
    ...
    MOTEUS_REG_MODE = 0x00
    MOTEUS_REG_POSITION = 0x01
    MOTEUS_REG_VELOCITY = 0x02
    MOTEUS_REG_TORQUE = 0x03
    MOTEUS_REG_Q_A = 0x04
    MOTEUS_REG_D_A = 0x05
    
    MOTEUS_REG_MOTOR_TEMP = 0x0a
    
    MOTEUS_REG_V = 0x0d
    MOTEUS_REG_TEMP = 0x0e
    MOTEUS_REG_FAULT = 0x0f
    ...
    MOTEUS_REG_VEL_LIM = 0x28
    MOTEUS_REG_ACC_LIM = 0x29
    
    MOTEUS_REG_ENC_2_POS = 0x54
...
class Controller:
    def __init__(self, controller_ID, device='/dev/fdcanusb'):
    ...
    def __send_can_frame(self, frame, reply, discard_adapter_response=True, print_data=False):
        ...
        self.serial.write("can send {:02x}{:02x} {}\n" .format(reply_indicator, self.target, self.__hexify(frame)).encode('utf8'))
        ...
        if discard_adapter_response:
            ...
            if reply:
                ...
                if print_data:
                    print("Mode: {: 2d}  Pos: {: 6.2f}rev  Vel: {: 6.2f}rps  "
                          "Torque: {: 6.2f}Nm    ".format(
                            int(response_data[MoteusReg.MOTEUS_REG_MODE]),
                            response_data[MoteusReg.MOTEUS_REG_POSITION],
                            response_data[MoteusReg.MOTEUS_REG_VELOCITY],
    ...
    def set_position_lim(self, position, velocity=0., max_torque=0.5, ff_torque=0., kp_scale=1., kd_scale=1., vel_lim=2., acc_lim=2.,
                     get_data=False, print_data=False):
        buf = io.BytesIO()
        buf.write(struct.pack(
            "<bbb",
        ...
        buf.write(struct.pack(
            "<bbff",
            0x0e,  # write float32 2x
            MoteusReg.MOTEUS_REG_VEL_LIM,
            vel_lim,
            acc_lim))
        if get_data:
            buf.write(struct.pack(
    ...
    def get_data_enc(self, print_data=False):
        buf = io.BytesIO()
        buf.write(struct.pack(
            "<bbb",
            0x1c,  # read float32 (variable number)
        ...
        buf.write(struct.pack(
            "<bb",
            0x1d,  # read float32 1x
            MoteusReg.MOTEUS_REG_ENC_2_POS))

        return self.__send_can_frame(buf.getvalue(), reply=True, print_data=print_data)
        
    def get_data_temp(self, print_data=False):
        buf = io.BytesIO()
        buf.write(struct.pack(
            "<bbb",
            0x1c,  # read float32 (variable number)
        ...
        buf.write(struct.pack(
            "<bb",
            0x11,  # read int8 1x
            MoteusReg.MOTEUS_REG_MOTOR_TEMP))
